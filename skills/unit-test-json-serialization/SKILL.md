...
class UserDtoJsonTest {
  ...
  void shouldSerializeUserToJson() throws Exception {
    ...
    assertThat(result).extractingJsonPathNumberValue("$.age").isEqualTo(25);
  }
...
class OrderJsonTest {
  ...
  void shouldIgnoreJsonIgnoreAnnotatedFields() throws Exception {
    ...
    assertThat(result).doesNotHaveJsonPathValue("$.internalNote");
  }
}
```
...
class UserListJsonTest {
  ...
  void shouldSerializeUserListToJson() throws Exception {
    ...
    assertThat(result).hasJsonPathStringValue("$[0].name");
    assertThat(result).hasJsonPathStringValue("$[1].name");
  }
}
```
...
class ProductJsonTest {
  ...
  void shouldSerializeNestedObjects() throws Exception {
    ...
    assertThat(result).extractingJsonPathStringValue("$.category.name").isEqualTo("Electronics");
  }
