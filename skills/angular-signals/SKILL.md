...
category: frontend
tags: [angular, signals, reactivity, computed, effect, state-management]
version: 1.0.0
...
- Managing reactive state with `signal()`, derived state with `computed()`, and dependent state with `linkedSignal()`
- Running side effects in response to state changes using `effect()`
- Converting between RxJS Observables and signals (`toSignal`, `toObservable`)
- Replacing `BehaviorSubject` or `Observable` patterns with signal-based reactivity
