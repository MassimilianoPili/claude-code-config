...
public class UserEventListener {
  ...
  public void onUserCreated(UserCreatedEvent event) {
    try {
      ...
      // Log error but don't propagate — event listeners should not break the publisher
    }
  }
}
...
class MultiListenerTest {
  ...
  void shouldNotifyMultipleListenersSequentially() {
    ...
    EmailService emailService2 = mock(EmailService.class);
    EmailService emailService3 = mock(EmailService.class);

    UserEventListener listener1 = new UserEventListener(emailService1);
    UserEventListener listener2 = new UserEventListener(emailService2);
    UserEventListener listener3 = new UserEventListener(emailService3);

    User user = new User(1L, "Alice", "alice@example.com");
    UserCreatedEvent event = new UserCreatedEvent(this, user);

    // Simulate Spring dispatching the same event to multiple listeners
    listener1.onUserCreated(event);
    listener2.onUserCreated(event);
    listener3.onUserCreated(event);

    verify(emailService1).sendWelcomeEmail("alice@example.com");
    verify(emailService2).sendWelcomeEmail("alice@example.com");
    verify(emailService3).sendWelcomeEmail("alice@example.com");
  }
}
```
