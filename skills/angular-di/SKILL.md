...
category: frontend
tags: [angular, dependency-injection, inject, providers, tokens]
version: 1.0.0
...
- Configuring dependency injection with `inject()` instead of constructor injection
- Creating and providing `InjectionToken` values for configuration or platform APIs
- Managing provider scopes (root singleton, component-level, route-level)
- Setting up app initializers, multi providers, or environment injectors
- Wiring up service dependencies with `useClass`, `useFactory`, or `useExisting`
